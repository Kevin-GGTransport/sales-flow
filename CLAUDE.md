...
- **代保管**（库存页 `/inventory?tab=custody`）：是 `Part.kind = CUSTODY` 的配件类型，与自营/寄卖共用 `Inventory` 和 `StockAdjustment`；可手动增加或消耗库存，不进加权平均/库存价值/报表，不可卖出/买入，消耗后不许为负数。与寄卖的区别：寄卖我们帮卖、代保管只记数量增减。旧 `/custody` 是 redirect 桩。
